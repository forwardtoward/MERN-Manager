  ...
  try {
    ...
    await membershipObj.save(async (err, data) => {
      if (err) {
        ...
        Shop.find({ userId: mongoose.Types.ObjectId(_id) }).exec(async (err, result) => {
          if (err) {
...
exports.addToWishlist = async (req, res) => {
  try {
    ...
    Membership.updateOne({ _id: id }, { $set: { isfavorite: 1 } }).exec(async (err, data) => {
      if (err) {
        res.send({
          msg: err,
          success: false,
...
exports.removeFromWishlist = async (req, res) => {
  try {
    let id = req.params.membershipId;
    Membership.updateOne({ _id: id }, { $set: { isfavorite: 0 } }).exec(async (err, data) => {
      if (err) {
        res.send({
          msg: err,
          success: false,
...
exports.membershipInfo = (req, res) => {
  try {
    ...
    Membership.find({ _id: mongoose.Types.ObjectId(membershipId) }).exec((err, data) => {
      if (err) {
...
exports.publicMembershipInfo = (req, res) => {
  try {
    let membershipId = req.params.membershipId;
    Membership.find({
...
exports.membershipPublicList = async (req, res) => {
  try {
    const perPage = req.query.perPage;
    const sortBy = req.query.sortBy;
    const page = req.query.page;
    ...
    const total = await Membership.find(options).count();
...
exports.membershipPublicInfo = (req, res) => {
  try {
    let membershipId = req.params.membershipId;
    const options = {
      _id: mongoose.Types.ObjectId(membershipId),
    };
    Membership.find(options).exec((err, data) => {
      if (err) {
        res.send({ msg: "membership  not found", success: false });
      } else {
        res.send({ data: data[0], success: true });
