...
const { FormEntry } = require("../models/index");
...
exports.getForms = async (req, res) => {
  try {
    ...
    id = mongoose.Types.ObjectId(id);
    let data;
    data = await FormBuilder.find({ userId: id });
    return res.status(200).json({ success: true, data });
  } catch (err) {
    res.send({ msg: "error" });
  }
...
exports.getTemplates = async (req, res) => {
  try {
    ...
    let orgs;
    let data;
    const userData = await FormBuilder.find({
      userId: mongoose.Types.ObjectId(id),
      isTemplate: true,
    }); // created by user
    const superAdmins = await FormBuilder.find({ isTemplate: true, creatorType: "super-admin" }); // created by super admin
    if (organization && organization.id !== null) {
      orgs = await FormBuilder.find({
        isTemplate: true,
        organizationId: mongoose.Types.ObjectId(organization_id),
        creatorType: "admin",
      }); // created by orgAdmin
      data = [...userData, ...superAdmins, ...orgs];
    } else {
      data = [...userData, ...superAdmins];
    }
