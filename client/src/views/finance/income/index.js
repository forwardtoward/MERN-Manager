...
const Income = () => {
  ...
  const last_incomeList = incomeList.filter((item) => new Date(item.date).getFullYear() == year && (new Date(item.date).getMonth() - 1) == currentMonth);

  const total = filtered_incomeList
    .map((item) => item.amount)
    .reduce((prev, current) => {
      return prev + current;
    }, 0);
  const k_total = total / 1000;
  const lastTotal = last_incomeList
    .map((item) => item.amount)
    .reduce((prev, current) => {
      return prev + current;
    }, 0);
      ...
      <Row className="match-height">
        <Col xl="4" md="6" xs="12">
          <Earnings 
            success={colors.success.main} 
            total = {total}
            filtered_list = {filtered_incomeList}
          />
        </Col>
        <Col xl="8" md="6" xs="12">
          <StatsCard 
