...
const Earnings = ({ success, total, filtered_list }) => {
    const colorData = [
    ...
    //donut chart data
    const donutColors = allCategoryByExpense.map((item) => item.labelColor).map((li) => colorData.filter((list) => list.color === li)[0].hex);
    const options = {
        chart: {
            toolbar: {
                show: false
        ...
        labels: allCategoryByExpense.map((item) => item.title),
        stroke: { width: 0 },
        colors: donutColors,
            ...
            <CardBody>
                <Row>
                    ...
                    <Col xs="6">
                        ...
                        <Chart options={options} series={allCategoryByExpense.map((item) => item.amount)} type='donut' height={120} />
                    </Col>
                </Row>
            </CardBody>
        </Card>
