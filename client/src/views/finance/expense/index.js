...
const Expense = () => {
  ...
  //select box data
  const categoryselect = ['Auto Payment', 'In House Payment', 'All Expense', 'Product'];
  const monthselect = [
    'Januray',
    'February',
    'March',
...
const Expense2 = () => {
  ...
  const last_expenseList = expenseList.filter((item) => new Date(item.date).getFullYear() == year && (new Date(item.date).getMonth() - 1) == currentMonth);
  const total = filtered_expenseList
    .map((item) => item.amount)
    .reduce((prev, current) => {
      return prev + current;
    }, 0);
  const k_total = total / 1000;
  const lastTotal = last_expenseList
    .map((item) => item.amount)
    .reduce((prev, current) => {
      return prev + current;
    }, 0);
  const handleWorkspaceCollapse = () => setCollapse(!collapse);
  console.log(total, filtered_expenseList)
  return (
    <div>
      <Row className="match-height">
        <Col xl="4" md="6" xs="12">
          <Earnings 
            success={colors.success.main} 
            total = {total}
            filtered_list = {filtered_expenseList}
            />
        </Col>
        <Col xl="8" md="6" xs="12">
          <StatsCard 
