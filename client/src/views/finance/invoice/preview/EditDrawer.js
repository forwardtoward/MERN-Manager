...
if(data){
  ...
  const handleSave = ()=>{
    ...
    formData.append("internalPaymentNote", data?.internalPaymentNote);
    formData.append("payNow", data?.payNow );
    formData.append("dueDate", data?.dueDate);
    formData.append("_id", data?._id);
    if(data?.file){
        formData.append("file", data?.file);
    }
    //console.log(formData.getAll('_id'))
    mutate(formData);
  }
            ...
            <Col sm={6} md={6} lg={6}>
              <div className="mb-1">
                ...
                <Input
                  id="balance"
                  name='invoiceBalance'
                  defaultValue={`Invoice Balance: $ ${(parseFloat(data?.totalAmount) + parseFloat(data?.tax || 0) - parseFloat(data?.discount || 0)) - (parseFloat(data?.paidAmount || 0))}`}
                  onChange={(e) => {
                    setData({
                      ...data,
                      [e.target.name]: e.target.value
