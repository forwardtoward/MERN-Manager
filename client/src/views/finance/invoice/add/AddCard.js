...
const AddCard = (props) => {
  const { invoicedata, setinvoice, payUsing, store } = props;
  // ** States
  ...
  const handlechnageitems = (e, i) => {
    if(e.target.value!=='selectItem'){
      ...
      if(name==='itemId'){
        ...
        data[i]['itemId'] = item.value;
        data[i]['description'] = item.desc;
        data[i]['rate'] = item.price;
        data[i]['quantity'] = 1;
      }
      else{
        data[i][name] = value;
  ...
  const handleTypeChanged = (e) => {
    ...
    switch (type) {
      case 'products':
        ...
        for (const item of store?.products) {
          list.push({value:item._id,name:item.product_name,price:item.product_price,desc:item.product_description})
        }
        // setinvoice({...invoicedata,items:[{
        //   itemId:list[0].value,
      ...
      case 'memberships':
        for (const item of store?.memberships) {
          list.push({value:item._id,name:item.product_name})
        }
        break;
      case 'events':
        break;
    ...
    setinvoice({
      ...
      // payNow:(list[0]?.price || 0) + (invoice?.tax || 0) - (invoice?.discount || 0),
      // totalAmount:list[0] || 0
    });
  };
  useEffect(() => {
    // ** Get Clients
