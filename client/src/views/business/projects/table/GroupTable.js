  ...
  const handleDragEnd = (result) => {
    ...
    updateColumnOrder(payload).then((response) => {
      if (response) {
        ...
        dispatch(updatedColumnOrder({ workspaceID:projectData.workspaceID ,  tableID:projectData._id ,  updatedMapOrder:response?.data?.updatedMapOrder}));
          ...
          <div className="d-flex align-items-center justify-content-between gap-2">
            ...
            >
              ...
              {rotateIcon && rowIds.length > 0 ? (
                <div className="cursor-pointer" onClick={toggleDelete}>
                ...
                {(provided) => (
                  <tr {...provided.droppableProps} ref={provided.innerRef}>
                    {columnTitle.map(({ id, column, _id }, index) =>
                      column === '_id' ? (
                        <th className="project-table-select" style={{ maxWidth: 'fit-content', paddingTop: '1.3rem' }}>
                  <CheckSquare size={20} />
                          </th>
                            ...
                            >
                              ...
                              >
                                <Input
                                  ...
                                  value={column}
                                  onChange={(e) => { handleChangeDynamicProperty(e, column, id) }}
                                  onKeyPress={(event) =>
                    ...
                    {rows?.length > 0 && (
                      <th
                        class="ml-4"
                        style={{ position: 'sticky', right: 0, paddingTop: '1.3rem', borderBottom: '1px solid #ebe9f1' }}
                      >
                        <div
                          className="cursor-pointer"
                          color="primary"
              ...
              {rows?.map((row, i) => (
                <tr className="project-table-row" key={row._id}>
                  {columnTitle.map(({ column: column, columnType: columnType }, j) => (
                    <td
                      className="project-table-fields"
                      key={j}
                      style={{ position: 'sticky', textAlign: 'center' }}
                    >
                      {column === '_id' ? (
                        <div>
                          <Input
                        ...
                        <Input
                          type="text"
                          value={row.dynamicProperties.reduce((acc, prop) => prop.columnName.toLowerCase() === column.toLowerCase() ? prop.value : acc, null)}
                        ...
                        <Flatpickr
                          ...
                          value={row.dynamicProperties.reduce((acc, prop) => prop.columnName.toLowerCase() === column.toLowerCase() ? prop.value : acc, null)}
                          onChange={(data) => { onSelectHandlerDynamicDate(data, row._id, column) }}
                          required
