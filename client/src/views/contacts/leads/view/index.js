...
const UserView = () => {
  // ** Store Vars
  const leadStore = useSelector((state) => state.leadContact);
  console.log(leadStore?.contactList?.data?.list);
    ...
    <div className="app-user-view">
      <Row>
        <Col xl="4" lg="5" xs={{ order: 1 }} md={{ order: 0, size: 5 }}>
          <LeadInfoCard
            selectedUser={user}
            notes={leadStore?.leadNote}
            leadId={leadStore?.contactList?.data?.list}
          />
