...
// action for lead notes
export const LeadNoteFetchAction = (_id) => async (dispatch) => {
  try {
    const { data } = await api.leadNoteList(_id);
    dispatch(leadNoteFetch(data));
  } catch (error) {}
};
export const LeadNoteAddAction = (newNote, _id) => async (dispatch) => {
  try {
    const { data } = await api.leadNoteAdd(newNote, _id);
    dispatch(LeadNoteFetchAction());
  } catch (error) {}
};
export const LeadNoteDeleteAction = (id) => async (dispatch) => {
  try {
    const { data } = await api.leadNoteDelete(id);
    dispatch(LeadNoteFetchAction());
  } catch (error) {}
};
export const LeadNoteEditAction = (newNote) => async (dispatch) => {
  try {
    const { data } = await api.leadNoteEdit(newNote);
    dispatch(LeadNoteFetchAction());
  } catch (error) {}
};
