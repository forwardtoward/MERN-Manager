...
const EmployeeInfo = () => {
  ...
  useEffect(() => {
    employeeAttance.data.map((eachAttendance, index) => {
      //   if (eachAttendance. == userData.id) {
      //     setCurrentAttendance(eachAttendance);
      //   }
    });
  }, [employeeAttance.data.list]);
    ...
    <div className="app-user-view">
      <Row>
        ...
        <Col xl="8" lg="7" xs={{ order: 0 }} md={{ order: 1, size: 7 }}>
          <UserTabs selectedEmployee={employee} active={active} toggleTab={toggleTab} />
        </Col>
      </Row>
    </div>
  ) : contact?.loading ? (
