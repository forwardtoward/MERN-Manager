...
export const useColumns = ({ setDeleteModal }, { toggle }, { setRow }, { fetchNotes }) => {
  // let count = [];
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        <span>{row?.position[0]?.position ? row?.position[0]?.position : row?.position}</span>
      )
    },
