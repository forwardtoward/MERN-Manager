...
//import { useGetEmployeeShift } from '../../../requests/contacts/employee-contacts';
import { getAllEmployeeShiftAction } from './store/actions';
import { contactUpdateByIdAction, contactListRequest } from '../employee/store/actions';
  ...
  useEffect(() => {
    ...
    empShowArrToMap.map((employee, index1) => {
      ...
      employeePosition.map((position, index2) => {
        if (employee?.position[0]?.position == position.position) {
          flag = 1;
        } else {
          return;
        }
  ...
  const handleDrop = (e, employeePosition, item) => {
    if (e.target.closest('th') && e.target.closest('th').getAttribute('id')) {
      ...
      if (shiftDragData.shiftId) {
        ...
        // let tmp = [];
        // tmp = arrToMap.map((employee, index1)=>{
        //   return employee.shift.map((eachShift, index2)=>{
        //     if(eachShift==shiftId){
        //       return {...employee, shift: [...shift, {}]}
        //     }
        //   })
        // })
        setShiftDragData({});
                ...
                <>
                  ...
                  {employeePosition.map((positionItem, positionIndex) => {
                    if (positionItem.category == categoryItem.category) {
                      return (
                        <tbody
                          onDragEnter={(e) => handleDragEnter(e, positionItem.position)}
                          onDragOver={handleDragOver}
                          onDragLeave={handleDragLeave}
                        ...
                        >
                          ...
                          {empShowArrToMap !== undefined &&
                            empShowArrToMap
                              .filter(
                                (item) => item?.position[0]?.position === positionItem.position
                              )
                              .map((item, index) => {
                                return (
                                  <tr
