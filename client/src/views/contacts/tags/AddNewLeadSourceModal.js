...
import { createLeadsSourceAction, deleteLeadsSourceAction, updateLeadsSourceAction } from '../store/actions';
...
export default function AddNewLeadSourceModal({ open, toggle, store, dispatch }) {
  ...
  const handleInputTitle = (e) => {
    ...
    setTitleValidation(store?.leadSources?.filter((x) => x.title === e.target.value).length === 0);
  };
