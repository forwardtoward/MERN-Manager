...
import {
  ...
  useAddContacts
  // useGetClientPosition
} from '../../../../requests/contacts/contacts';
...
// ** Note Actions
import { contactNoteDeleteAction, contactNoteFetchAction } from '../../store/actions';
...
const UsersList = (props) => {
  ...
  const tableData = contactList?.filter((x) => x.contactType.indexOf(contactTypeId) > -1);
  ...
  function fetchData() {
    ...
    dispatch(contactNoteFetchAction());
  }
  // ** Get data on mount
  useEffect(() => {
    fetchData();
  ...
  const handleRowSelected = ({ selectedRows }) => {
    if (selectedRows.length > 0) {
      setSelectedRowDataProg(selectedRows);
    } else {
      setShowdelete(false);
    }
          ...
          <DataTable
            ...
            subHeaderComponent={
              <CustomHeader
                ...
                dispatch={dispatch}
                selectedRowDataProg={selectedRowDataProg}
                setSelectedRowDataProg={setSelectedRowDataProg}
              />
            }
      ...
      >
        ...
        <ModalFooter>
          ...
          <Button
            ...
            color="primary"
            onClick={() => {
              dispatch(contactNoteDeleteAction(deleteModal?.id));
              setDeleteModal({
                id: '',
                show: false
              });
