...
import { addClientProgressionAction } from '../../store/actions';
import { useDispatch } from 'react-redux';

const ProgressionTemplate = (props) => {
  const { stepper, setRSelected, rSelected, userIdSelected } = props;

  const [payloadRedux, setPayloadRedux] = useState({});
  // const payloadRedux =  {"clientIds":userIdSelected}
  ...
  useEffect(() => {
    rSelected === 1 ? dispatch(addClientProgressionAction(payloadRedux)) : null;
  }, [rSelected]);
  return (
    <Fragment>
      {/* <Card className="p-2"> */}
