...
import { setOnEditableContactAction, setOnShowAddNewActionSideBarAction, setAddedParentAction, setActions } from '../../store/actions';
    ...
    <div style={{ padding: 'auto', alignItems: 'center' }}>
      ...
      <div style={{ borderRadius: '50%', margin: 'auto', padding: "auto", width: '110px', height: '55px', cursor: 'pointer' }} >
        ...
        <div style={{ margin: 'auto', padding: 'auto' }}>
          {data.actionType == 'email' && data.attachments.length == 0 && (
            <img src={sendEmailSvg} alt="email" style={{ position: 'absolute', marginLeft: '-28px' }} height="50" width="165" />
          )}
          {data.actionType == 'email' && data.attachments.length > 0 && <img src={watchedVideoSvg} alt="email" style={{ position: 'absolute', marginLeft: '-28px' }} height="50" width="165" />}
          {data.actionType == 'text' && (
            <img src={sendTextSvg} alt="text" height="50" width="165" style={{ position: 'absolute', marginLeft: '-28px' }} />
          )}
          {data.actionType == 'notification' && (
            <img src={followUpSvg} alt="note" height="50" width="165" style={{ position: 'absolute', marginLeft: '-28px' }} />
          )}
          {data.actionType == 'automation' && (
            <img src={automationSvg} alt="automation" height="50" width="165" style={{ position: 'absolute', marginLeft: '-28px' }} />
          )}
      ...
      <div>
        {data.actionType != 'condition' && <><p style={{ marginBottom: '0px', textAlign: 'center', fontWeight: 'bold' }}>{data.actionType == 'editContact' ? 'START' : data.actionType.toUpperCase()}</p>
          {data.actionType == 'editContact' ? <p style={{ marginBottom: '0px', fontSize: '11px', textAlign: 'center', color: 'transparent' }}>fd</p> : <p style={{ marginBottom: '0px', fontSize: '11px', textAlign: 'center', color: 'rgb(242, 132, 34)' }}>Immediately</p>}</>}
      ...
      {isCondition ?
        ...
        : <>
          {data.actionType == "condition" ? <div style={{ cursor: 'pointer', position: 'absolute', width: '20px', height: '20px', background: 'white', border: '2px solid #b1b1b7', borderRadius: " 50%", zIndex: "999", left: '45px', top: '100px' }} onClick={() => addNewNode()}>
            <p style={{ position: 'absolute', fontSize: '28px', top: '-2px' }}>+</p>
          </div> : <div style={{ cursor: 'pointer', position: 'absolute', width: '20px', height: '20px', background: 'white', border: '2px solid #b1b1b7', borderRadius: " 50%", zIndex: "999", left: '45px', top: '125px' }} onClick={() => addNewNode()}>
            <p style={{ position: 'absolute', fontSize: '28px', top: '-2px' }}>+</p>
          </div>}
        </>}
