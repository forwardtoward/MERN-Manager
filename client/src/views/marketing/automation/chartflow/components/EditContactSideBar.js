...
const EditContactSideBar = ({ open, toggleSidebar }) => {
  ...
  const [activationUpon, setActivationUpon] = useState(null);
  ...
  useEffect(() => {
    if (automation.isEdit) {
      const selectedAutomationContact = automation.selectedAutomation.contactInfo;
      if (selectedAutomationContact != {}) {
        setContactType(selectedAutomationContact.contactType);
        // setActivateTime(selectedAutomationContact.activateTIme);
        setSelectedContact(selectedAutomationContact.selectedContact);
        setSmartList(selectedAutomationContact.smartlist);
        setActivationUpon(selectedAutomationContact.activationUpon);
      }
    }
  }, []);
