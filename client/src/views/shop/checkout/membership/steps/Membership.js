...
import { buyMembership } from '../../../store';
...
// ** Actions
import { checkoutSetBuyMembership } from '../../../store';
...
const Membership = (props) => {
  ...
  // ** Props
  const { stepper, dispatch, getMembership, setMembershipDetail, contact } = props;
  // ** Vars
  const {
    control,
    setError,
      ...
      .then((res, err) => {
        if (res && res.payload) {
          ...
          if (result.status === 'success') {
            dispatch(checkoutSetBuyMembership(result.data));              
            stepper.next();
          } else {
